...
def _request_profile(sensor_type, device):
    if sensor_type == CONF_MOTION and device == "pir":
        return "motion"
    return None
...
async def async_setup_platform(hass, config, async_add_entites, discovery_info=None):
    ...
    for device_config in config[CONF_DEVICES]:
        ...
        sensor = Sensor(
            ...
            device=device,
            request_profile=_request_profile(sensor_type, device),
            diagnostic_role=sensor_type,
