...
async def async_setup_platform(hass, config, async_add_entites, discovery_info=None):
    ...
    for address, device_config in config[CONF_DEVICES].items():
        ...
        _LOGGER.debug(
            "Adding cover '{}' with address {} and channel number {} and is adjustable {}".format(name, device_address,
                                                                                                  channel_number,
                                                                                                  _adjustable))
...
class BusproCover(CoverEntity):
    ...
    async def async_added_to_hass(self):
        """Subscribe and read state only after Home Assistant adds the entity."""
        self.async_register_callbacks()
        self.async_on_remove(
            event.async_track_time_interval(
                self.hass, self.async_update, self._polling_interval
            )
        )
    ...
    def async_register_callbacks(self):
        ...
        self._device.register_device_updated_cb(self._after_update_callback)
        self.async_on_remove(
            lambda: self._device.unregister_device_updated_cb(
